...
fn recurse_hashless_min_point<'a>(
    ...
    let all_added: Vec<(Added, AddIx)> = padded
        .list_addixs()
    ...
    let unique_addeds: std::collections::HashMap<_, _, FxBuildHasher> = all_added
        ...
        .map(|(added, add_ix)| {
            ...
            let (t, a) = Transformed::from_cropped_min_point(&cropped, add_ix);
            ((t, a), added)
        })
        .collect();

    'added: for ((added_transformed, add_ix), added) in unique_addeds {
        let aps = std::cell::OnceCell::<ArticulationPoints>::new();
        ...
        for (ix, b) in added_transformed.array_view().indexed_iter() {
            ...
            if ix.into_pattern() >= add_ix.into_pattern() {
                break;
            }

            if aps
                .get_or_init(|| ArticulationPoints::from_array(added_transformed.array_view()))
                .can_remove(ix)
            {
                continue 'added;
            }
...
mod ap {
    ...
    #[allow(dead_code)]
    fn search_connected_ignore_start(array: ArrayView3<bool>, to_remove: Ix3) -> usize {
        ...
        CACHE.with(|rc| {
            ...
            seen.clear();
            seen.resize(array.len(), false);
            let mut seen =
                ndarray::ArrayViewMut3::from_shape(array.raw_dim(), seen.as_mut_slice()).unwrap();
            ...
            to_see.push(
                neighbor_ixs(to_remove)
                    .into_iter()
                    .find(|&ix| *array.get(ix).unwrap_or(&false))
                    .unwrap(),
            );
            ...
            while let Some(node) = to_see.pop() {
                ...
                for neighbor in neighbor_ixs(node) {
                    if array.get(neighbor).copied().unwrap_or(false) && !seen[neighbor] {
