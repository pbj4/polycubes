...
use {
    added::*, ap::*, bb::*, com::*, comb::*, cropped::*, dim::*, n::*, padded::*, perm::*,
    transformed::*,
};
...
fn recurse_hashless_min_point<'a>(
    ...
    let unique_addeds = {
        let dup = all_added.iter().map(|(added, add_ix)| {
            ...
            (
                ...
                padded_aps.crop_transform(cropped.bb_used(), t.transform_used()),
            )
        });
    ...
    'added: for ((add_ix, added_transformed), transformed_aps) in unique_addeds {
        ...
        if possible_smaller.clone().any(|ix| {
