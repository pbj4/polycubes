...
fn recurse_hashless<'a>(
    ...
    let unique_added: HashMap<(Transformed, Ix3), &Added, FxBuildHasher> = all_added
        .iter()
        .map(|(added, add_ix)| {
            let (cropped, offset) = Cropped::from_added(added);
            let (t, p, c) = Transformed::from_cropped(&cropped);
            let add_ix = transform_ix(add_ix.ix() - offset, p, c, cropped.dim());

            ((t, add_ix), added)
        })
        .collect();

    println!("unique_added: {unique_added:#?}");

    for ((added_transformed, add_ix), added) in unique_added {
        // everything happens in canonical space
        let aps = ArticulationPoints::from_array(added_transformed.array_view());

        debug_assert!(added_transformed.array_view()[add_ix]);
        ...
        if add_ix
            == added_transformed
                ...
                .indexed_iter()
                .find_map(|(ix, &b)| {
                    let ix = array3_to_ix(ix.into());
                    if b && aps.can_remove(ix) {
                        Some(ix)
                    } else {
                        None
                    }
                ...
                .unwrap()
        {
            ...
            s.spawn(move |s| {
                let (canonical_added, mirror_symmetry) =
                    Transformed::from_cropped_check_sym(&Cropped::from_added(&added).0);
        ...
        /*
        debug_assert!(test_swap(
            added.array_view(),
            aps.clone().into_ap_array().view()
        ));
        */
...
mod cropped {
    ...
    impl<'a> Cropped<'a> {
        pub fn from_added(added: &'a Added) -> (Self, Ix3) {
        ...
        pub fn transform_array(
            ...
            comb.invert_array(perm.permute_array(self.view))
        }
        */
    }
...
mod transformed {
    ...
    impl<'a> Transformed<'a> {
                    ...
                    for c in Combination::all() {
                        ...
                        if precomp_com.is_valid(precomp_com_cc) {
                            let new = Self {
                                ...
                                view: transform_array(cropped.array_view(), p, c),
                                com: precomp_com,
                                n,
                            };
        ...
        pub fn from_cropped(cropped: &Cropped<'a>) -> (Self, Permutation, Combination) {
            let mut lowest: Option<(Self, Permutation, Combination)> = None;
            let n = cropped.n();
                ...
                if precomp_dim.is_valid() {
                    for c in Combination::all() {
                        ...
                        if precomp_com.is_valid(precomp_com_cc) {
                            ...
                            if let Some(prev_lowest) = lowest {
                                lowest = Some(prev_lowest.min((new, p, c)));
                            } else {
                                // fast path
            ...
            let (new, canon_perm, canon_comb) = lowest.unwrap();
            new.verify_com();
            new.verify_n();
            (new, canon_perm, canon_comb)
        }
    ...
    impl CenterOfMass {
        ...
        pub fn from_array(array: ArrayView3<bool>) -> Self {
            Self {
                ix: array
                    .indexed_iter()
                    .filter_map(|(ix, &b)| b.then_some(array3_to_ix(ix.into())))
                    .reduce(|a, b| a + b)
                    .unwrap(),
            }
        }
...
mod bb {
    ...
    impl BoundingBox {
        ...
        pub fn min(&self) -> Ix3 {
            array3_to_ix(self.min)
        }

        #[allow(dead_code)]
        pub fn max(&self) -> Ix3 {
            array3_to_ix(self.max)
        }
    }
}
    ...
    impl ArticulationPoints {
        ...
        pub fn into_ap_array(self) -> Array3<bool> {
            Array3::from_shape_vec(
                ...
                self.states
                    .indexed_iter()
                    .map(|(ix, _)| self.post_is_ap(array3_to_ix(ix.into())))
                    .collect(),
            )
            .unwrap()
        }
...
fn array3_to_ix(array: [usize; 3]) -> Ix3 {
    let [x, y, z] = array;
    Ix3(x, y, z)
}
...
fn transform_array(
    ...
    comb.invert_array(perm.permute_array(view))
}

fn transform_ix(ix: Ix3, perm: Permutation, comb: Combination, dim: Dimensions) -> Ix3 {
    comb.invert_ix(
        perm.permute_ix(ix),
        perm.permute_ix(dim.ix() - Ix3(1, 1, 1)),
    )
}
...
fn test_articulation_impl() {
    fn test<const N: usize>(shape: (usize, usize, usize), array: [bool; N], expected: [bool; N]) {
        let array = ArrayView3::from_shape(shape, &array).unwrap();
        let expected = ArrayView3::from_shape(shape, &expected).unwrap();

        assert_eq!(
            ArticulationPoints::from_array(array).into_ap_array(),
            expected,
            "original: {array:?}"
        );

        test_swap(array, expected);
    }
...
#[allow(dead_code)]
fn test_swap(array: ArrayView3<bool>, expected: ArrayView3<bool>) -> bool {
    ...
    for (ix, &is_ap) in expected.indexed_iter() {
        ...
        let mut array3 = array.to_owned();
        array3[ix] = false;

        let calculated_ap = ArticulationPoints::from_array(array3.view()).update_add_node(ix);
        let calculated = calculated_ap.clone().into_ap_array();
        ...
        assert_eq!(
            calculated
                ...
                .filter(|p| *p.1)
                .collect::<Vec<_>>(),
            expected.indexed_iter().filter(|p| *p.1).collect::<Vec<_>>(),
            "original: {array:?}\ndifference: {:?}\nstate: {:?}",
            (calculated.clone() ^ expected)
                .indexed_iter()
                .filter(|p| *p.1)
                .collect::<Vec<_>>(),
            calculated_ap
