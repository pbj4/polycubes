...
fn recurse_hashless<'a>(
    ...
    let unique_addeds: std::collections::HashMap<_, _, FxBuildHasher> = all_added
        ...
        .map(|(added, add_ix)| {
            (
                Transformed::from_cropped_canonical_only_no_sym(&Cropped::from_added(added)),
                (added, *add_ix),
            )
        })
        .collect();
    ...
    'added: for (_added_transformed, (added, add_ix)) in unique_addeds {
        ...
        s.spawn(move |s| {
            let (canonical_added, mirror_symmetry) =
                Transformed::from_cropped_canonical_only_2(&Cropped::from_added(&added));
...
mod com {
    ...
    impl CenterOfMass {
        ...
        pub fn classify_octant(&self, com_cc: CenterOfMassCalcCache) -> [std::cmp::Ordering; 3] {
            let (cx, cy, cz) = self.ix.into_pattern();
            let (dx, dy, dz) = com_cc.dn.into_pattern();

            [(2 * cx).cmp(&dx), (2 * cy).cmp(&dy), (2 * cz).cmp(&dz)]
        }
...
mod perm {
    ...
    impl Permutation {
        ...
        pub fn all_valid(inverted_com: CenterOfMass, dim: Dimensions) -> Vec<Self> {
            ...
            idc.sort_unstable_by_key(|[_, _, c]| *c);
            idc.sort_by_key(|[_, d, _]| *d);

            let mut i = idc.map(|[i, _add_ix, _]| i);
...
mod comb {
    ...
    impl Combination {
        ...
        pub fn all_valid_octant(com: CenterOfMass, com_cc: CenterOfMassCalcCache) -> Vec<Self> {
            ...
            out.push(ords.map(|o| o.is_gt())); // base
