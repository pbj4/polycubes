    ...
    let unique_addeds = {
        let mut vec: Vec<_> = all_added
            ...
            .map(|(added, add_ix)| {
                ...
                let (t, t_add_ix) = Transformed::from_cropped_min_point_2(&cropped, add_ix);
                let transformed_aps = t.transform_used().transform_view(cropped_aps);
                ((t_add_ix.into_pattern(), t), (added, transformed_aps))
            })
            .collect();

        // stable sort seems to have better performance here
        vec.sort_by_key(|(k, _)| *k);
        vec.dedup_by_key(|(k, _)| *k);
        vec
    };

    'added: for ((add_ix, added_transformed), (added, transformed_aps)) in unique_addeds {
        let add_ix = usize3_to_ix(add_ix.into());
...
mod transformed {
    ...
    impl<'a> Transformed<'a> {
        pub fn from_cropped_canonical(cropped: &Cropped<'a>) -> (Self, bool) {
            ...
            thread_local! {
                static CACHE: RefCell<(Vec<Combination>, Vec<Permutation>)> = RefCell::default();
            }
        ...
        pub fn from_cropped_min_point_2(cropped: &Cropped<'a>, ix: Ix3) -> (Self, Ix3) {
