...
mod cropped {
    ...
    pub struct Cropped(Array3<bool>, N);
...
mod padded {
    ...
    pub struct Padded(Array3<bool>, N);
...
mod added {
    ...
    pub struct Added(Array3<bool>, (Ix3, Ix3), N);
        ...
        pub fn from_cropped(cropped: &Cropped) -> impl Iterator<Item = Self> + '_ {
            Permutation::all().flat_map(move |p| {
                Combination::all().filter_map(move |c| {
                    ...
                    if !CenterOfMass::is_valid(&CenterOfMass::from_array(&transformed), &dim, &n) {
                        None
                    } else {
