...
fn recurse_parallel_rotations<'a>(
    ...
    let canon = Transformed::from_cropped_canonical(cropped).unwrap_or_else(|| {
        let mut transformations = Transformed::from_cropped(cropped).peekable();
        let (first, r1) = transformations.peek().unwrap().clone();
...
mod transformed {
    ...
    impl Transformed {
        pub fn from_cropped(cropped: &Cropped) -> impl Iterator<Item = (Self, bool)> + '_ {
            Permutation::all()
                .filter_map(move |p| {
                    let n = cropped.n();
