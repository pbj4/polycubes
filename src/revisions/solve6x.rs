...
pub fn solve(n: usize) -> BTreeMap<usize, (usize, usize)> {
    ...
    let out: Vec<_> = std::iter::repeat_with(Default::default).take(n).collect();
...
fn recurse_hashless_min_point<'a>(
    ...
    let unique_addeds = collect_dedup(all_added.iter().map(|(added, add_ix)| {
    ...
    for ((t_add_ix, added_transformed), (bb_used, original_add_ix)) in unique_addeds {
...
mod ap_semi_streaming {
    ...
    #[allow(dead_code)]
    pub struct ApStreamingBorrowed<'a> {
        nodes: &'a mut [NodeData],
    }

    #[allow(dead_code)]
    impl<'a> ApStreamingBorrowed<'a> {
        pub fn from_owned(owned: &'a mut ApStreaming) -> Self {
            Self {
                nodes: owned.nodes.as_mut_slice(),
            }
        }

        pub fn from_owned_multiple(
            owned: &'_ ApStreaming,
            vec: &'a mut Vec<NodeData>,
            copies: usize,
        ) -> Vec<Self> {
            vec.clear();
            vec.reserve(owned.nodes.len() * copies);
            vec.extend_from_slice(owned.nodes.as_slice());

            for _ in 0..copies {
                vec.extend_from_within(0..owned.nodes.len());
            }

            vec.chunks_exact_mut(owned.nodes.len())
