...
pub fn solve(n: usize) -> BTreeMap<usize, (usize, usize)> {
    let (base, mirror_sym) = Transformed::from_cropped_canonical_only(&Cropped::default());
...
fn recurse_hashless<'a>(
    ...
    let unique_addeds = {
        let mut canonical_addeds: Vec<(Transformed, &Added, AddIx)> = all_added
            .iter()
            .map(|(added, add_ix)| {
                (
                    Transformed::from_cropped_canonical_only(&Cropped::from_added(added)).0,
                    added,
                    *add_ix,
                )
            })
        ...
        canonical_addeds.sort_by_key(|(transformed, _, _)| *transformed);
        canonical_addeds.dedup_by_key(|(transformed, _, _)| *transformed);
        canonical_addeds
    };

    for (_added_transformed, added, add_ix) in unique_addeds {
        let all_removed: Vec<Removed> = remove_ixs
            .iter()
            .filter_map(|remove_ix| Removed::from_added(added, *remove_ix))
            .collect();

        let canonical_removed = all_removed
            .iter()
            .map(|removed| {
                Transformed::from_cropped_canonical_only(&Cropped::from_removed(
                    removed,
                    add_ix,
                    remove_ixs.as_slice(),
                ))
        ...
        if transformed.cmp_with_lifetime(&canonical_removed).is_le() {
            let added = added.clone();
            s.spawn(move |s| {
                let (canonical_added, mirror_symmetry) =
                    Transformed::from_cropped_canonical_only(&Cropped::from_added(&added));
...
mod removed {
    ...
    impl Removed {
        pub fn from_added(added: &Added, remove_ix: RemoveIx) -> Option<Self> {
            ...
            let visited_nodes = search_connected(array.view(), added.add_ix().ix());

            if visited_nodes == n.num() {
                let new = Self {
                    array,
                    n,
                    remove_ix,
...
mod dim {
    ...
    impl Dimensions {
        ...
        pub fn from_cropped_permutate(cropped: &Cropped, perm: Permutation) -> Self {
            Self {
                ix: perm.reorder_ix(cropped.dim().ix()),
            }
        }
    }
    ...
    impl CenterOfMass {
        ...
        pub fn from_array(array: ArrayView3<bool>) -> Self {
            Self {
                ix: array
                    .indexed_iter()
                    .filter_map(|(ix, &b)| b.then_some(array_to_ix(ix.into())))
                    .reduce(|a, b| a + b)
                    .unwrap(),
            }
        }
...
mod bb {
    ...
    impl BoundingBox {
        ...
        pub fn min(&self) -> Ix3 {
            array_to_ix(self.min)
        }

        #[allow(dead_code)]
        pub fn max(&self) -> Ix3 {
            array_to_ix(self.max)
        }
    }
}
...
fn array_to_ix(array: [usize; 3]) -> Ix3 {
    let [x, y, z] = array;
    Ix3(x, y, z)
}

fn search_connected(array: ArrayView3<bool>, start: Ix3) -> usize {
    thread_local! {
        static CACHE: RefCell<(HashSet<Ix3, FxBuildHasher>, Vec<Ix3>)> = RefCell::default();
    }
    ...
    CACHE.with(|rc| {
        ...
        while let Some(node) = to_see.pop() {
            ...
            for neighbor in neighbor_ixs(node) {
                if !seen.contains(&neighbor) && array.get(neighbor).copied().unwrap_or(false) {
